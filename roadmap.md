...
|------|--------------|-----------|----------------|--------|
| S0.1 | `specs/spec-S0.1-install-node/` | — | Install Node.js and npm. Run `node --version` to confirm | pending |
| S0.2 | `specs/spec-S0.2-create-project/` | S0.1 | Create the project with Vite. Run `npm run dev`. See "Hello World" in browser | pending |
| S0.3 | `specs/spec-S0.3-git-setup/` | S0.2 | Create a GitHub repo. Push the starter project. Understand what `.gitignore` does | pending |
| S0.4 | `specs/spec-S0.4-js-for-python-devs/` | S0.1 | Learn the 6 JavaScript patterns used throughout this project. Compare each to its Python equivalent | pending |
| S0.5 | `specs/spec-S0.5-react-fundamentals/` | S0.2, S0.4 | Learn what React is, what a component is, what JSX is, and what `useState` does — by experimenting in the live project | pending |
...
| Spec | Spec Location | Depends On | What You'll Do | Status |
|------|--------------|-----------|----------------|--------|
| S1.1 | `specs/spec-S1.1-html-skeleton/` | S0.2 | Create `index.html` with a text input, a button, and a response area | pending |
| S1.2 | `specs/spec-S1.2-js-click-handler/` | S1.1 | Write JavaScript so clicking the button shows the typed text on screen | pending |
| S1.3 | `specs/spec-S1.3-basic-styling/` | S1.2 | Add Tailwind CSS. Make it look clean — centred, readable, not ugly | pending |
...
| Spec | Spec Location | Depends On | What You'll Do | Status |
|------|--------------|-----------|----------------|--------|
| S2.1 | `specs/spec-S2.1-get-api-keys/` | S0.1 | Sign up at Groq, Gemini, and OpenRouter. Get a free API key from each. Test all three with curl | pending |
| S2.2 | `specs/spec-S2.2-first-api-call/` | S2.1, S1.2 | Write a `fetch()` call to Groq's API. Show the AI's reply on screen | pending |
| S2.3 | `specs/spec-S2.3-error-handling/` | S2.2 | Handle "no key entered", "rate limit hit", and "no internet" gracefully | pending |
...
| Spec | Spec Location | Depends On | What You'll Do | Status |
|------|--------------|-----------|----------------|--------|
| S3.1 | `specs/spec-S3.1-component-architecture/` | S0.2, S2.2 | Clean up `App.jsx` into a single-responsibility component — the foundation for splitting into `MessageThread` and `ChatInput` in S3.2/S3.3 | pending |
| S3.2 | `specs/spec-S3.2-message-thread/` | S3.1 | Build `<MessageThread>` component. Messages stored in a list. New messages append | pending |
| S3.3 | `specs/spec-S3.3-chat-input/` | S3.2, S2.2 | Build `<ChatInput>` component. Enter to send. AI reply appears in thread | pending |
...
| Spec | Spec Location | Depends On | What You'll Do | Status |
|------|--------------|-----------|----------------|--------|
| S4.1 | `specs/spec-S4.1-sidebar-layout/` | S3.3 | Add a sidebar panel next to the chat. Controls go here | pending |
| S4.2 | `specs/spec-S4.2-temperature-slider/` | S4.1 | Add temperature slider (0–2). Wire to API call. See responses change | pending |
| S4.3 | `specs/spec-S4.3-top-p-slider/` | S4.2 | Add top-p slider (0.1–1.0). Wire to API call. Understand how top-p differs from temperature | pending |
| S4.4 | `specs/spec-S4.4-max-tokens-slider/` | S4.3 | Add max tokens slider (64–2048). Wire to API call. See how response length is capped | pending |
| S4.5 | `specs/spec-S4.5-provider-picker/` | S4.4 | Add provider dropdown (Groq / Gemini / OpenRouter). Switching it changes which API is called | pending |
...
| Spec | Spec Location | Depends On | What You'll Do | Status |
|------|--------------|-----------|----------------|--------|
| S5.1 | `specs/spec-S5.1-tokeniser-logic/` | S0.1 | Install `gpt-tokenizer`. Write a function that splits text into tokens. Test it | pending |
| S5.2 | `specs/spec-S5.2-tokeniser-ui/` | S5.1, S3.1 | Build the token chip UI. Each token gets a different colour. Show token count | pending |
...
| Spec | Spec Location | Depends On | What You'll Do | Status |
|------|--------------|-----------|----------------|--------|
| S6.1 | `specs/spec-S6.1-routing/` | S3.1 | Install React Router. Add `/chat` and `/learn` routes. Nav bar links between them | pending |
| S6.2 | `specs/spec-S6.2-pretrain-panel/` | S6.1 | Build pre-training section: Data collection, Cleaning, Tokenisation, Architecture cards | pending |
| S6.3 | `specs/spec-S6.3-postrain-panel/` | S6.1 | Build post-training section: SFT, RLHF, PPO, Verifiable tasks cards | pending |
| S6.4 | `specs/spec-S6.4-eval-gen-panels/` | S6.1 | Build Evaluation and Text Generation sections. Embed the tokeniser demo inline | pending |
...
| Spec | Spec Location | Depends On | What You'll Do | Status |
|------|--------------|-----------|----------------|--------|
| S7.1 | `specs/spec-S7.1-compare-route/` | S6.1 | Add `/compare` route. Two-column layout. Each column has its own provider | pending |
| S7.2 | `specs/spec-S7.2-parallel-calls/` | S7.1, S2.2 | Send the prompt to both providers at the same time using `Promise.all` | pending |
| S7.3 | `specs/spec-S7.3-response-cards/` | S7.2 | Show each response in a card with provider name, token count, and latency | pending |
...
| Spec | Spec Location | Depends On | What You'll Do | Status |
|------|--------------|-----------|----------------|--------|
| S8.1 | `specs/spec-S8.1-eval-route/` | S6.1 | Add `/evaluate` route. Two sub-sections: rate responses, view benchmarks | pending |
| S8.2 | `specs/spec-S8.2-star-rating/` | S8.1, S3.2 | Star rating widget for each AI response. Stores ratings in browser memory | pending |
| S8.3 | `specs/spec-S8.3-benchmark-chart/` | S8.1 | Bar chart showing MMLU / HumanEval / GSM8K scores for each free model | pending |
...
| Spec | Spec Location | Depends On | What You'll Do | Status |
|------|--------------|-----------|----------------|--------|
| S9.1 | `specs/spec-S9.1-dark-mode/` | S1.3 | Add a dark/light mode toggle. Preference saved across page refreshes | pending |
| S9.2 | `specs/spec-S9.2-mobile-layout/` | S3.3 | Sidebar collapses on small screens. Chat input sticks to the bottom | pending |
| S9.3 | `specs/spec-S9.3-empty-error-states/` | S2.3 | Nice empty state when chat is empty. Clear error messages when API fails | pending |
...
| Spec | Spec Location | Depends On | What You'll Do | Status |
|------|--------------|-----------|----------------|--------|
| S10.1 | `specs/spec-S10.1-prod-build/` | S9.1, S9.2, S9.3 | Run `npm run build`. Understand what a "production build" is | pending |
| S10.2 | `specs/spec-S10.2-vercel-deploy/` | S10.1 | Deploy to Vercel (free). Get a public URL. Share it | pending |
...
|------|-------|----------------|--------|
| S0.1 | Tools Setup | Install Node.js and npm | pending |
| S0.2 | Tools Setup | Create project with Vite | pending |
| S0.3 | Tools Setup | Push to GitHub | pending |
| S0.4 | Tools Setup | JavaScript fundamentals for Python developers | pending |
| S0.5 | Tools Setup | React fundamentals — components, JSX, useState | pending |
| S1.1 | First Web Page | HTML skeleton | pending |
| S1.2 | First Web Page | JS click handler | pending |
| S1.3 | First Web Page | Basic Tailwind styling | pending |
| S2.1 | Connect to AI | Get free API keys: Groq + Gemini + OpenRouter | pending |
| S2.2 | Connect to AI | First real AI call | pending |
| S2.3 | Connect to AI | Error handling | pending |
| S3.1 | Chat UI | App component architecture checkpoint | pending |
| S3.2 | Chat UI | Message thread | pending |
| S3.3 | Chat UI | Chat input | pending |
| S4.1 | Controls | Sidebar layout | pending |
| S4.2 | Controls | Temperature slider | pending |
| S4.3 | Controls | Top-p slider | pending |
| S4.4 | Controls | Max tokens slider | pending |
| S4.5 | Controls | Provider picker (Groq / Gemini / OpenRouter) | pending |
| S5.1 | Tokeniser | Tokeniser logic | pending |
| S5.2 | Tokeniser | Token chip UI | pending |
| S6.1 | Learn Panel | Routing | pending |
| S6.2 | Learn Panel | Pre-training cards | pending |
| S6.3 | Learn Panel | Post-training cards | pending |
| S6.4 | Learn Panel | Evaluation + Generation cards | pending |
| S7.1 | Compare | Compare route layout | pending |
| S7.2 | Compare | Parallel AI calls | pending |
| S7.3 | Compare | Response cards | pending |
| S8.1 | Evaluate | Evaluate route | pending |
| S8.2 | Evaluate | Star rating widget | pending |
| S8.3 | Evaluate | Benchmark chart | pending |
| S9.1 | Polish | Dark mode | pending |
| S9.2 | Polish | Mobile layout | pending |
| S9.3 | Polish | Empty + error states | pending |
| S10.1 | Deploy | Production build | pending |
| S10.2 | Deploy | Vercel deploy | pending |
